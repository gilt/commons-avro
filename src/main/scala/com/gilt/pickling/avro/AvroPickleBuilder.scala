...
final class AvroPickleBuilder(format: AvroPickleFormat, out: AvroEncodingOutput) extends PBuilder with PickleTools {
  ...
  @inline private[this] def mkByteBuffer(): Unit =
    if (byteBuffer == null)
      byteBuffer = new AvroEncodingOutput()
  ...
  @inline def beginEntry(picklee: Any): PBuilder = withHints {
    hints =>
      ...
      hints.tag.key match {
        ...
        case KEY_NONE => byteBuffer.encodeLongTo(0)
        case key if key.startsWith(KEY_SOME) => byteBuffer.encodeLongTo(1) // TODO be nice to match against TypeRef
        case key =>
